      ...
      {isOpen && (
        <div data-lenis-prevent="true" className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-4 md:p-6 overflow-y-auto pointer-events-auto">
          ...
          <motion.div
            ...
            className="relative my-auto w-full max-w-[280px] sm:max-w-[340px] md:max-w-[380px] aspect-[4/5] max-h-[78vh] sm:max-h-[480px] min-h-[300px] bg-[#0A2850] rounded-[1.5rem] sm:rounded-[2rem] border border-white/15 overflow-hidden shadow-[0_20px_70px_rgba(0,0,0,0.85)] z-10 flex flex-col justify-end pointer-events-auto"
            id="admissions-popup-card"
          >
            {/* Elegant Floating Close Button */}
            <button
              onClick={onClose}
              className="absolute top-3 right-3 sm:top-4 sm:right-4 z-20 p-2 rounded-full bg-black/60 text-white border border-white/10 hover:bg-white hover:text-slate-900 transition-all duration-300 cursor-pointer hover:rotate-90 active:scale-95"
            ...
            <div className="absolute bottom-0 inset-x-0 p-5 sm:p-6 flex flex-col items-center justify-end z-10 text-center">
              <span className="font-mono text-[9px] sm:text-[10px] font-bold tracking-[0.25em] text-blue-300 uppercase mb-4 sm:mb-5">
              ...
              <button
                ...
                className="w-full bg-[#1E3A8A] text-white hover:bg-[#1D4ED8] hover:text-white py-3 sm:py-3.5 px-5 rounded-xl text-[11px] sm:text-[12px] font-bold tracking-[0.2em] uppercase flex items-center justify-center gap-2 transition-all duration-300 cursor-pointer shadow-lg shadow-black/40 hover:shadow-xl hover:shadow-black/60 border border-white/15 active:scale-95 hover:scale-[1.02]"
              >
                <span>Apply Now</span>
